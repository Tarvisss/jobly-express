...
router.post("/users/:username/jobs/:id", 
  ensureLoggedIn, isUserOrAdmin, async function (req, res, next) {
    try{
      const jobId = Number(req.params.id);
      await User.applyToJob(req.params.username, jobId);
      return res.json({msg: `Applied for job id:${jobId}`})
    }catch (err) {
      return next(err);
    }
  });
...
router.get("/", ensureLoggedIn, isUserOrAdmin, async function (req, res, next) {
  try {
